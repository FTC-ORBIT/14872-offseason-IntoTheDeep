...
// convert -180-180 in Radians
    ...
    public static float wrapAccordingTo(final float refrence, final float target) {
        return refrence + Angle.wrapPlusMinusPI(target - refrence);
    }

    public static boolean inTolerance(final float theta, final float alpha, final float tolerance) {
        return Math.abs(wrapPlusMinusPI(theta - alpha)) < tolerance;
    }
